...
// ====================== ЛОГИН ======================
$PASSWORD = "odifar78"; // ← ИЗМЕНИ СРАЗУ!
$TITLE = t('app_name');
...
<body class="bg-zinc-950 text-zinc-100 min-h-screen pb-20">
    ...
    <div id="multiBar" class="hidden fixed bottom-0 inset-x-0 bg-zinc-900 border-t border-zinc-700 p-4 flex items-center justify-between z-[100] safe-bottom">
        ...
        <button onclick="clearSelection()" class="ml-4 text-zinc-400 font-medium px-6 py-5"><?= t('cancel') ?></button>
    </div>
    ...
    <script>
        ...
        function getFileVisual(item) {
            ...
            const map = {
                // ЗОБРАЖЕННЯ
                'jpg':  {icon: 'image', color: 'text-pink-400', badge: 'JPG'},
                'jpeg': {icon: 'image', color: 'text-pink-400', badge: 'JPG'},
                'png':  {icon: 'image', color: 'text-pink-400', badge: 'PNG'},
                'gif':  {icon: 'gif', color: 'text-pink-400', badge: 'GIF'},
                'webp': {icon: 'image', color: 'text-pink-400', badge: 'WEBP'},
                'svg':  {icon: 'image', color: 'text-pink-400', badge: 'SVG'},
                // PDF
                ...
                // КОД + ТЕКСТ
                'txt':  {icon: 'description', color: 'text-zinc-400', badge: 'TXT'},
                'html': {icon: 'html', color: 'text-orange-400', badge: 'HTML'},
                'css':  {icon: 'css', color: 'text-blue-400', badge: 'CSS'},
                'js':   {icon: 'javascript', color: 'text-yellow-400', badge: 'JS'},
        ...
        async function loadFiles() {
            ...
            if (currentPath) {
                ...
                backRow.innerHTML = `
                    <span class="material-symbols-outlined text-blue-400 text-[42px] flex-shrink-0">arrow_back_ios</span>
                    <button onclick="goBack();event.stopImmediatePropagation()" class="list-name-btn text-blue-400">.. ${backText}</button>
                `;
            ...
            data.items.forEach(item => {
                ...
                row.innerHTML = `
                    ...
                    <div class="flex items-center gap-3 text-right flex-shrink-0">
                        ...
                        ${!item.dir ? `<div class="text-xs text-zinc-400 font-medium whitespace-nowrap">${(item.size/1024).toFixed(1)} KB</div>` : ''}
                    </div>
                `;
